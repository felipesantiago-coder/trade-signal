...
optimize_15m.py
--------------
Otimizador de grid search dedicado ao timeframe 15m para BTC/USDT.

Diferencas vs optimize_v5.py (1h):
  - Timeframe 15m: 4x mais candles, mais ruido, movimentos menores em %
  - Custos relativos maiores (~0.35% por trade vs ~0.09% em 1h)
  - Estrategia mista: Trend-Following + Mean-Reversion no mesmo grid
  - Max bars reduzido (72 candles 15m = 18h, vs 72 candles 1h = 3 dias)
  - Espaco de parametros adaptado: RSI mais justo, SL mais curto,
    ATR percentile mais restritivo
  - Candles/dia = 96 (vs 24 no 1h)

Abordagem:
  Phase 1: Grid massivo com ~5000+ combinacoes
  Phase 2: Refinamento fino dos top 15 (SL/TP/MR params)
  Phase 3: Validacao com custos realistas
"""
...
SYMBOL = "BTC/USDT"
TIMEFRAME = "15m"
DAYS = 365  # 1 ano de dados (Binance limit for 15m ~ 1 year)
CANDLES_PER_DAY = 96  # 24h * 4
DATA_CACHE = "/tmp/ctev_15m_data.pkl"

# Metricas minimas adaptadas para 15m
MIN_TRADES_P1 = 80    # Mais trades necessarios para significancia estatistica
MIN_TRADES_P2 = 150
MIN_WIN_RATE = 48.0   # Levemente relaxado vs 1h (50%)
MAX_DRAWDOWN = 20.0   # Mais restritivo

# Custo round-trip tipico em 15m: ~0.35%
# Spread 10bps + Slippage 20bps + Fees 0.025%*2 = 0.05% total
# Em 15m, slippage real pode ser maior por ser mais rapido
FEE_15M = 0.025
SPREAD_15M = 10.0
SLIPPAGE_15M = 25.0  # 25bps — mais realista para 15m
...
class Optimizer15m:
    def __init__(self):
        ...
        self.vol_a = self.vs50 = self.vs20 = None
        self.e20t = self.e50t = self.e50tu = None
        self.bb_lower = self.bb_upper = self.bb_width = self.bb_squeeze = None
        self.df_clean = None
        self.buy_hold = 0.0
        self.idx_arr = None
    ...
    def load_data(self):
        if os.path.exists(DATA_CACHE):
            logger.info("Carregando cache: %s", DATA_CACHE)
            with open(DATA_CACHE, 'rb') as f:
                self.df_clean = pickle.load(f)['df_clean']
        else:
            logger.info("Baixando dados %s %s (%d dias)...", SYMBOL, TIMEFRAME, DAYS)
            df = fetch_historical_ohlcv(SYMBOL, TIMEFRAME, DAYS)
            logger.info("Calculando indicadores para %d candles...", len(df))
            ...
            crit = ["ema20", "ema50", "ema200", "rsi", "atr", "atr_percentile",
                    ...
                    "regime", "bb_lower", "bb_upper", "bb_width", "bb_squeeze_pct"]
            self.df_clean = df_ind.dropna(subset=crit).copy()
            with open(DATA_CACHE, 'wb') as f:
                pickle.dump({'df_clean': self.df_clean}, f)
            logger.info("Cache salvo: %s (%d candles limpos)", DATA_CACHE, len(self.df_clean))
        ...
        self.vs20 = df['volume_sma20'].values.astype(np.float64)
        ...
        self.bb_squeeze = df['bb_squeeze_pct'].values.astype(np.float64)
        self.buy_hold = (self.close[-1] - self.close[0]) / self.close[0] * 100
        rc = dict(zip(*np.unique(self.regime, return_counts=True)))
        logger.info(
            "%d candles (%d dias) | B&H=%.2f%% | %s",
            self.n, self.n // CANDLES_PER_DAY, self.buy_hold, rc
        )
    ...
    def _trend_long(self, i, p):
        ...
        if rg == 'trending_up':
            ...
            if not p.get('allow_ranging_trend', False): return None
        else: return None

        c = self.close[i]
        e50 = self.ema50_a[i]
        e200 = self.ema200_a[i]

        if p['require_ema_trend'] and not (c > e50 > e200): return None
        if p['require_slope'] and self.eslope[i] <= p.get('slope_min', -1.0): return None
        ...
        if p['require_pullback']:
            ...
            # EMA20 proximity
            if not pb and p.get('ema20_prox_pct', 0) > 0 and e20 > 0:
                if abs(c - e20) / e20 <= p['ema20_prox_pct']: pb = True
            # EMA50 proximity
            if not pb and p.get('ema50_prox_pct', 0) > 0 and e50 > 0:
        ...
        if p.get('volume_confirm', False):
            v = self.vs50[i]
            if not np.isnan(v) and v > 0 and self.vol_a[i] < v * p.get('volume_sma_ratio', 0.3): return None

        ap = self.atr_pct[i]
        if ap < p['atr_pct_min'] or ap > p['atr_pct_max']: return None

        av = self.atr_a[i]; ep = c
        sl = ep - p['sl_atr_mult'] * av
        tp = ep + p['tp_atr_mult'] * av
    ...
    def fast_sim(self, p, max_bars=72):
        ...
        allow_rng_trend = p.get('allow_ranging_trend', False)
        ...
        while i < n:
            ...
            if rg == 'ranging':
                if mr_on:
                    ...
                    if sig: is_long = True
                        ...
                        sig = self._mr_short(i, p)
                        is_long = False
                elif allow_rng_trend:
                    sig = self._trend_long(i, p)
                    if sig: is_long = True
                    else:
                        sig = self._trend_short(i, p)
                        is_long = False
            ...
            _, adj, _ = _apply_costs(ep, exit_price, is_long,
                                     FEE_15M, SPREAD_15M, SLIPPAGE_15M)
    ...
    def run_combo(self, cid, p, phase="p1", max_bars=72):
        ...
        r = GridResult(
            combo_id=cid,
            allow_ranging=p.get('allow_ranging_trend', False),
    ...
    def _score(r, phase="p1"):
        ...
        # Freq bonus: normalizado para trades por dia (96 candles/dia em 15m)
        days = 365
        freq_bonus = math.log(max(r.total_trades, 1)) / math.log(max(days * 0.5, 1))

        # PnL bonus
        pnl_b = 2.0 if r.total_pnl_pct > 10.0 else (1.5 if r.total_pnl_pct > 5.0 else (1.2 if r.total_pnl_pct > 0 else 0.5))
...
def make_p1_grid():
    """
    Grid massivo Phase 1 para 15m.
    Estrategias testadas:
    A) Trend-Following puro (sem MR)
    B) Trend + Mean-Reversion (MR no ranging)
    C) Trend relaxado (sem pullback, sem slope)
    D) Apenas Mean-Reversion (MR dominant)
    """
    combos = []

    # ── A) Trend-Following configs ──
    # (allow_trans, require_pullback, require_ema_trend, require_slope, adx_min,
    #  rsi_long_range, rsi_short_range)
    trend_cfgs = [
        # Config baseline: identico ao 1h v5.0
        (True,  True,  True,  True,  25, (28,48), (55,75)),
        # Relax ADX — 15m tem menos tendencia forte
        (True,  True,  True,  True,  20, (28,48), (55,75)),
        (True,  True,  True,  True,  15, (28,48), (55,75)),
        # Sem slope — mais sinais
        (True,  True,  True,  False, 20, (28,50), (53,75)),
        (True,  True,  True,  False, 25, (25,50), (50,75)),
        # Sem pullback — focus em RSI + regime
        (True,  False, True,  False, 20, (30,55), (45,70)),
        (True,  False, True,  False, 25, (30,55), (45,70)),
        # Sem EMA trend — mais sinais
        (True,  True,  False, False, 20, (30,55), (45,70)),
        (True,  False, False, False, 15, (35,60), (40,65)),
        # RSI mais justo para 15m
        (True,  True,  True,  True,  20, (25,45), (55,75)),
        (True,  True,  True,  True,  20, (30,50), (50,70)),
        # RSI alargado
        (True,  True,  True,  False, 20, (25,55), (45,75)),
        (True,  False, True,  False, 20, (25,55), (45,75)),
        # Config com volume
        (True,  True,  True,  False, 20, (28,50), (50,72)),
    ]

    # SL/TP para trend
    tr_sltp = [
        (0.75, 2.0), (1.0, 2.5), (1.0, 3.0), (1.25, 2.5),
        (1.25, 3.0), (1.5, 3.0), (1.5, 3.5), (0.75, 1.5),
        (1.0, 2.0), (1.25, 4.0),
    ]

    # MR configs
    mr_rsi_l = [30, 35, 40, 45]
    mr_rsi_s = [55, 60, 65, 70]
    mr_bb = [0.0, 0.005, 0.01]
    mr_sltp = [
        (0.5, 1.0), (0.75, 1.5), (1.0, 1.5), (0.75, 2.0),
        (1.0, 2.0), (1.0, 2.5), (0.5, 1.25),
    ]
            ...
            if mr_on:
                    ...
                    for mrs in mr_rsi_s:
                        for mbb in mr_bb:
                            for sl, tp in mr_sltp:
                                combos.append({
                                    'allow_ranging_trend': False,
                                    'allow_volatile': False,
                                    'allow_transition': a_tr,
                                    'require_pullback': r_pb,
                                    'require_ema_trend': r_et,
                                    'require_slope': r_sl,
                                    'slope_min': -1.0 if r_sl else -99.0,
                                    'rsi_long_min': rsi_l[0],
                                    'rsi_long_max': rsi_l[1],
                                    'rsi_short_min': rsi_s[0],
                                    'rsi_short_max': rsi_s[1],
                                    'adx_min': adx,
                                    'fib_tolerance_pct': 0.025,
                                    'ema20_prox_pct': 0.0,
                                    'ema50_prox_pct': 0.0,
                                    'volume_confirm': False,
                                    'volume_sma_ratio': 0.3,
                                    'atr_pct_min': 0.05,
                                    'atr_pct_max': 0.95,
                                    'mr_enabled': True,
                                    'mr_rsi_long_max': float(mrl),
                                    'mr_rsi_short_min': float(mrs),
                                    'mr_bb_band_pct': mbb,
                                    'sl_atr_mult': sl,
                                    'tp_atr_mult': tp,
                                })
                ...
                for sl, tp in tr_sltp:
                    combos.append({
                        ...
                        'atr_pct_max': 0.95,
                        'mr_enabled': False,
                        'mr_rsi_long_max': 35.0,
                        'mr_rsi_short_min': 65.0,
                        'mr_bb_band_pct': 0.0,
                        'sl_atr_mult': sl,
                        'tp_atr_mult': tp,
                    })

    # ── D) MR-only configs (trend signals disabled) ──
    for mrl in mr_rsi_l:
        for mrs in mr_rsi_s:
            for mbb in mr_bb:
                for sl, tp in mr_sltp:
...
def make_p2_grid(top, n=15):
    """Refinamento Phase 2: foca em SL/TP e MR params."""
    sls = [0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0, 2.5]
    tps = [1.0, 1.25, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0]
    mr_rsi_l = [28, 32, 35, 38, 42, 45, 50]
    mr_rsi_s = [50, 55, 58, 62, 65, 68, 72]
    mr_bb = [0.0, 0.003, 0.005, 0.008, 0.01, 0.015, 0.02]

    combos = []
    for r in top[:n]:
        base = {
            'allow_ranging_trend': r.allow_ranging,
            'allow_volatile': r.allow_volatile,
            'allow_transition': r.allow_transition,
            'require_pullback': r.require_pullback,
            'require_ema_trend': r.require_ema_trend,
            'require_slope': r.require_slope,
            'rsi_long_min': r.rsi_long_min,
            'rsi_long_max': r.rsi_long_max,
            'rsi_short_min': r.rsi_short_min,
            'rsi_short_max': r.rsi_short_max,
            'adx_min': r.adx_min,
            'fib_tolerance_pct': r.fib_tolerance_pct,
            'ema20_prox_pct': r.ema20_prox_pct,
            'ema50_prox_pct': r.ema50_prox_pct,
            'volume_confirm': r.volume_confirm,
            'volume_sma_ratio': r.volume_sma_ratio,
            'atr_pct_min': r.atr_pct_min,
...
def _save(winner, all_res, opt, t0, p1c, p2c):
    ...
    if all_res:
        ...
        df_r.to_csv(os.path.join(out, "grid_15m_results.csv"), index=False)
    if winner:
        wdata = {
            "version": "15m-optimized-v1",
            "timeframe": "15m",
            "symbol": SYMBOL,
            "days": DAYS,
            "score": winner.score,
            "params": {k: getattr(winner, k) for k in [
                'allow_ranging', 'allow_volatile', 'allow_transition',
                'require_ema_trend', 'require_slope', 'require_pullback',
                'mr_enabled', 'mr_rsi_long_max', 'mr_rsi_short_min',
                'mr_bb_band_pct', 'rsi_long_min', 'rsi_long_max',
                'rsi_short_min', 'rsi_short_max', 'adx_min',
                'fib_tolerance_pct', 'ema20_prox_pct', 'ema50_prox_pct',
                'sl_atr_mult', 'tp_atr_mult', 'volume_confirm',
                'volume_sma_ratio', 'atr_pct_min', 'atr_pct_max',
                'max_bars']},
            "metrics": {
                ...
                "avg_win_pct": winner.avg_win_pct,
                "avg_loss_pct": winner.avg_loss_pct,
                "best_trade_pct": winner.best_trade_pct,
                "worst_trade_pct": winner.worst_trade_pct,
                "avg_bars_held": winner.avg_bars_held,
            },
            "costs": {
                "fee_pct": FEE_15M,
                "spread_bps": SPREAD_15M,
                "slippage_bps": SLIPPAGE_15M,
            },
            "grid": {"p1": p1c, "p2": p2c, "total": p1c + p2c,
...
def main():
    t0 = time.time()
    print("\n" + "#" * 160)
    print("#  CTEV 15m OPTIMIZER — Grid Search para Timeframe 15 Minutos")
    print("#  Objetivo: Encontrar estrategia lucrativa para 15m BTC/USDT")
    print("#  Custo realista: fee=%.3f%% spread=%.0fbps slippage=%.0fbps" % (FEE_15M, SPREAD_15M, SLIPPAGE_15M))
    print("#" * 160)
    ...
    for idx, params in enumerate(p1):
        r = opt.run_combo(idx, params, "p1", max_bars=72)
        p1_res.append(r)
        if r.score > 0:
            viable += 1
    ...
    if not p1_sorted or p1_sorted[0].score == 0:
        ...
        print("\nTop 10 por PnL (sem filtros de score):")
        ...
        for r in by_pnl[:10]:
            print(f"  T={r.total_trades} WR={r.win_rate:.1f}% PF={r.profit_factor:.2f} "
                  f"PnL={r.total_pnl_pct:+.2f}% DD={r.max_drawdown_pct:.2f}% MR={'S' if r.mr_enabled else 'N'}")
        return
    ...
    for idx, params in enumerate(p2):
        r = opt.run_combo(200000 + idx, params, "p2", max_bars=72)
        p2_res.append(r)
        if r.score > best_score:
            best_score = r.score
    ...
    if winner:
        ...
        print(f"#  ADX:           {winner.adx_min:.0f} | Max Bars: {winner.max_bars}")
