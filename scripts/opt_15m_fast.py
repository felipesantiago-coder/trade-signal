'''CTEV 15m Fast Optimizer — SIMPLE, FAST, WALK-FORWARD
~600 combos, no trailing, no BE, simple SL/TP exit
Realistic costs: 0.072% RT
'''
...
print(f'{n}c ({n//96}d) B&H={bh:+.2f}% {rc}', flush=True)
...
def sim(entries, dirs, sls, tps, eps, mb):
    trades = []
    for k in range(len(entries)):
        ...
        for j in range(idx+1, mj):
            if is_l:
                ...
                if lo[j]<=sl: xp=sl; bars=j-idx; break
            else:
                if lo[j]<=tp: xp=tp; bars=j-idx; break
                if hi[j]>=sl: xp=sl; bars=j-idx; break
...
def gen(strat, start, end, p):
    ...
    adx_min=p.get('adx_min',0); cd=p['cooldown']; mb=p['max_bars']
    i=start; cooldown=0
    while i<end:
        if i<cooldown: i+=1; continue
        rg=regime[i]
        if rg=='volatile' or i<1: i+=1; continue
        ci=c[i]; li=lo[i]; hii=hi[i]; r=rsi_a[i]; a=ap_a[i]
        ...
        if strat=='momentum':
            ...
            if rg=='ranging' and bbsq[i]<=0.20:
                if hii>=bbu[i] and rl[0]<=r<=rl[1] and rd_a[i]>0 and ci>e50[i]:
                    is_l=True; sig=True
                elif li<=bbl[i] and rs[0]<=r<=rs[1] and rd_a[i]<0 and ci<e50[i]:
                    is_l=False; sig=True
            ...
            if rg in ('trending_up','transition') and adx_a[i]>=adx_min:
                if li<=ref[i]*1.001 and ci>ref[i]*0.998 and ci>e50[i]>e200[i]:
                    if rl[0]<=r<=rl[1] and (mh[i]>0 or rd_a[i]>0):
            ...
            if rg in ('trending_down','transition') and adx_a[i]>=adx_min:
                if hii>=ref[i]*0.999 and ci<ref[i]*1.002 and ci<e50[i]<e200[i]:
                    if rs[0]<=r<=rs[1] and (mh[i]<0 or rd_a[i]<0):
                ...
                for cd in [4,8]:
                    for adx in [0,20]:
                        grid.append({'strat':'momentum','rsi_l':rl,'rsi_s':rs,'sl_m':sl,'tp_m':tp,
                            'max_bars':mb,'cooldown':cd,'adx_min':adx})
            ...
            for mb in [24,48]:
                for cd in [4,8]:
                    grid.append({'strat':'squeeze','rsi_l':rl,'rsi_s':rs,'sl_m':sl,'tp_m':tp,
                        'max_bars':mb,'cooldown':cd,'adx_min':0})
                    ...
                    for ue in [20,50]:
                        for adx in [0,20]:
                            grid.append({'strat':'pullback','rsi_l':rl,'rsi_s':rs,'sl_m':sl,'tp_m':tp,
                                'max_bars':mb,'cooldown':cd,'adx_min':adx,'use_ema':ue})
...
for idx, p in enumerate(grid):
    st=p['strat']; mb=p['max_bars']
    ea,da,sa,ta,epa = gen(st, 0, n, p)
    if len(ea)<100: continue
    et,dt,st2,tt2,ept = gen(st, 0, mid, p)
    ete,dte,ste,tte,epte = gen(st, mid, n, p)
    ma=calc_m(sim(ea,da,sa,ta,epa,mb))
    mt=calc_m(sim(et,dt,st2,tt2,ept,mb))
    mte=calc_m(sim(ete,dte,ste,tte,epte,mb))
    if not ma: continue
...
for i, r in enumerate(results[:20]):
    ...
    print(f'{i+1:>3} {r.score:>7.3f} {r.strategy:>9} {r.total_trades:>6} {ls:>6} {td:>5.2f} {r.win_rate:>6.1f} {r.profit_factor:>6.2f} '
          ...
          f'{r.p["sl_m"]:>4.2f} {r.p["tp_m"]:>4.2f} {r.p["max_bars"]:>3} {r.p["cooldown"]:>3} {r.p.get("adx_min",0):>3}', flush=True)
